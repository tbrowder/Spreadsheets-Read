...
=table
$s.cell(0,2) | row, col (zero-indexed)
$s.rowcol(0,2) | row, col (zero-indexed)
$s.colrow(2,0) | col, row (zero-indexed)
$s.cell('c1') | Excel A1 syntax
$s.cell('C1') | Excel A1 syntax
...
empty rows and remaining rows are trimmed to remove trailing empty cells.
